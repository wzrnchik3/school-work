...
void Autocomplete::search(string key, int& firstIndex, int& lastIndex)
{
    ...
    else
    {
        ...
        lastIndex = found_pos;
        for (int i = found_pos; i >= 0; i--){ // finds initial index
            keyToMatch = terms[i].query.substr(0, key.length());
            if (key == keyToMatch)
                firstIndex = i;
        }
        for (int j = found_pos; j < terms.size(); j++){
            keyToMatch = terms[j].query.substr(0, key.length());
            if (key == keyToMatch)
                lastIndex = j;
        }
    }
